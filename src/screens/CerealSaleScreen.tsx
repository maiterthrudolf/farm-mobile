...
import { addCerealSale, fetchCerealSaleStats, CerealStatRow } from '../api/client';
import DatePickerField from '../components/DatePickerField';

type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'CerealSale'> };
...
export default function CerealSaleScreen({ navigation }: Props) {
  const { lang } = useLang();
  const ro = lang === 'RO';
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const s = await fetchCerealSaleStats();
  ...
  const resetForm = () => {
    setCerealType(''); setTStr(''); setClient('');
  ...
  const handleSubmit = async () => {
    ...
    if (!client.trim()) { setError(ro ? 'Introdu clientul' : 'Enter client name'); return; }
    setLoading(true);
    try {
      await addCerealSale({ cereal_type: cerealType, sale_kg: t * 1000, client: client.trim(), sale_date: saleDate });
      setTStr(''); setClient('');
      loadStats();
      setShowDetails(true);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <NavHeader
        title={ro ? 'Vanzare Cereale' : 'Sale Cereals'}
      ...
      {showDetails ? (
          ...
          {statsLoading
            ...
            : <>
                ...
                <View style={[styles.statsSection, styles.statsSectionGreen]}>
                  <Text style={styles.statsSectionTitleGreen}>{ro ? 'Total Vanzari' : 'Total Sales'} {new Date().getFullYear()}</Text>
                  {CEREAL_TYPES.map(ct => {
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Text style={styles.label}>{ro ? 'Data Vanzare' : 'Date of Sale'}</Text>
          ...
          <Text style={styles.label}>{ro ? 'Client' : 'Client'}</Text>
          <TextInput
            style={styles.input} value={client} onChangeText={setClient}
            placeholder={ro ? 'Numele clientului' : 'Client name'} placeholderTextColor={Colors.border}
          />
          ...
          {loading
            ...
            : (
              <TouchableOpacity style={styles.addBtn} onPress={handleSubmit} activeOpacity={0.8}>
                <Text style={styles.addBtnTxt}>{ro ? 'Adauga la Vanzare' : 'Add to Sale'}</Text>
              </TouchableOpacity>
            )
          }
        </ScrollView>
