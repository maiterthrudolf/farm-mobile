...
import { searchMotherByLast4, MotherInfo } from '../api/client';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'BirthMother'>;
};



export default function BirthMotherScreen({ navigation }: Props) {
  ...
  const [results, setResults] = useState<MotherInfo[] | null>(null);
  ...
  const proceed = (mother: MotherInfo) => {
    navigation.navigate('BirthScan', { company: mother.company, mother });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const all = await searchMotherByLast4(last4);
      const females = all.filter((r: MotherInfo) => r.sex === 'F');
      if (females.length === 0) {
        setError(ro ? 'Crotaliu nu este valid' : 'No female animal found with this ID');
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {results && (
            <View style={styles.resultsSection}>
              <Text style={styles.resultsHint}>
                {ro ? `${results.length} rezultate — selecteaza mama:` : `${results.length} results — select the mother:`}
              </Text>
              {results.map(m => (
                <AnimalCard key={m.ear_tag} data={m} lang={lang} onPress={() => proceed(m)} showGestation={true} />
              ))}
          ...
          {!results && (
            <>
              <FormField label={ro ? 'Crotaliu MAMA - cel putin ultimele 4 cifre:' : 'Mother ID - at least the last 4 digits:'}>
                <TextInput
              ...
              <TouchableOpacity
                style={styles.noMotherBtn}
                onPress={() => navigation.navigate('BirthCompany')}
                activeOpacity={0.8}
              >
                <Text style={styles.noMotherBtnTxt}>{ro ? 'Fara Mama' : 'Without Mother'}</Text>
              </TouchableOpacity>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  noMotherBtn:       { backgroundColor: '#E65100', borderRadius: 8, paddingVertical: 16, alignItems: 'center' },
  noMotherBtnTxt:    { color: '#fff', fontSize: 17, fontWeight: '700' },
