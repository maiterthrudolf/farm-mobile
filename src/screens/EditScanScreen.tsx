...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'EditScan'>;
};

export default function EditScanScreen({ navigation }: Props) {
  ...
  const [results, setResults] = useState<CattleSearchResult[] | null>(null);
  ...
  const proceed = (animal: CattleSearchResult) => {
    navigation.navigate('EditDetails', { earTag: animal.ear_tag });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const animals = await searchByLast4(last4);
      if (animals.length === 0) {
          ...
          {results && (
            <View style={styles.resultsSection}>
              <Text style={styles.resultsHint}>
                {ro
                  ? `${results.length} rezultate — selecteaza animalul:`
                  : `${results.length} results — select the animal:`}
              </Text>
              {results.map(a => (
