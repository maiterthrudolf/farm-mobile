...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'IDCompany'>;
};
...
export default function IDCompanyScreen({ navigation }: Props) {
  const { lang, t } = useLang();
  const ro = lang === 'RO';

  return (
    <SafeAreaView style={styles.safe}>
      <NavHeader title={t('id')} onBack={() => navigation.goBack()} />
      ...
      <View style={styles.content}>
        {COMPANIES.map(c => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('IDScan', { company: c.key })}
            activeOpacity={0.8}
          >
