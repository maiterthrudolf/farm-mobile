...
import { searchSaleAnimalByLast4, SaleAnimalInfo } from '../api/client';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'SaleScan'>;
  route: RouteProp<RootStackParamList, 'SaleScan'>;
};

function parseError(detail: string, ro: boolean): string {
  if (detail.startsWith('LIST:')) {
    const listName = detail.slice(5);
    return ro ? `Animal deja pe lista: ${listName}` : `Animal already on list: ${listName}`;
  }
  const u = detail.toUpperCase();
  if (u === 'MORT')   return ro ? 'Crotaliu nu este valid: Mort'   : 'No valid ID: Dead';
  if (u === 'VANDUT') return ro ? 'Crotaliu nu este valid: Vandut' : 'No valid ID: Sold';
  return ro ? 'Crotaliu nu este valid' : 'No valid ID';
}

export default function SaleScanScreen({ navigation, route }: Props) {
  ...
  const [results, setResults] = useState<SaleAnimalInfo[] | null>(null);
  ...
  const proceed = (animal: SaleAnimalInfo) => {
    navigation.navigate('SaleAnimal', { listId, listName, animal });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const animals = await searchSaleAnimalByLast4(last4);
      if (animals.length === 0) {
      ...
      setError(parseError(e?.message ?? '', ro));
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.listBanner}>
        <Text style={styles.listBannerTxt}>{listName}</Text>
      </View>
          ...
          {results && (
            <View style={styles.resultsSection}>
              <Text style={styles.resultsHint}>
                {ro ? `${results.length} rezultate — selecteaza:` : `${results.length} results — select:`}
              </Text>
              {results.map(a => (
          ...
          {!results && (
            <>
              <Text style={styles.label}>
                {ro ? 'introdu cel putin ultimele 4 cifre:' : 'Enter at least the last 4 digits:'}
              </Text>
              <TextInput
              ...
              {/* Lista button — below Cauta */}
              <TouchableOpacity
                style={styles.listBtn}
                onPress={() => navigation.navigate('SaleList', { listId, listName })}
                activeOpacity={0.8}
              >
                <Text style={styles.listBtnTxt}>{ro ? 'Lista' : 'List'}</Text>
              </TouchableOpacity>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  listBanner:        { backgroundColor: Colors.primaryLight, paddingHorizontal: 16, paddingVertical: 8 },
  listBannerTxt:     { fontSize: 14, fontWeight: '700', color: Colors.primaryDark, textAlign: 'center' },
  ...
  listBtn:           { backgroundColor: Colors.primary, borderRadius: 8, paddingVertical: 16, alignItems: 'center' },
  listBtnTxt:        { color: '#fff', fontSize: 17, fontWeight: '700' },
