...
import { searchDeathAnimalByLast4, DeathAnimalInfo } from '../api/client';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'DeathScan'>;
};

export default function DeathScanScreen({ navigation }: Props) {
  ...
  const [results, setResults] = useState<DeathAnimalInfo[] | null>(null);
  ...
  const proceed = (animal: DeathAnimalInfo) => {
    navigation.navigate('DeathDetails', { animal });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const animals = await searchDeathAnimalByLast4(last4);
      if (animals.length === 0) {
        setError(ro ? 'Crotaliu nu este valid' : 'No valid ID found');
      ...
      if (detail === 'already_dead') {
        setError(ro ? 'Animal deja mort' : 'Already dead');
      } else if (detail === 'already_sold') {
        setError(ro ? 'Animal deja vandut' : 'Already sold');
      } else {
        setError(detail || (ro ? 'Server indisponibil' : 'Server unavailable'));
      }
    } finally {
      setLoading(false);
      setScanned(false);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <NavHeader title={t('deaths')} onBack={() => navigation.goBack()} />
