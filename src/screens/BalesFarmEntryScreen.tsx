...
import { addBaleEntry, fetchBaleEntryStats, BaleStatRow } from '../api/client';
import DatePickerField from '../components/DatePickerField';

type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'BalesFarmEntry'> };
...
export default function BalesFarmEntryScreen({ navigation }: Props) {
  const { lang } = useLang();
  const ro = lang === 'RO';
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const s = await fetchBaleEntryStats();
  ...
  const resetForm = () => {
    setBaleType(''); setWeight(null); setCount(''); setIsPurchased(false);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await addBaleEntry({ bale_type: baleType, weight_kg: weight, count: cnt, is_purchased: isPurchased, entry_date: entryDate });
      setCount('');
      await loadStats();
      setShowDetails(true);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <NavHeader
        title={ro ? 'Intrare Ferma Baloti' : 'Farm Entry Bales'}
      ...
      {showDetails ? (
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <TouchableOpacity
            style={[styles.checkRow, isPurchased && styles.checkRowSel]}
            onPress={() => setIsPurchased(v => !v)}
          >
            <View style={[styles.checkbox, isPurchased && styles.checkboxSel]}>
              {isPurchased && <Text style={styles.checkmark}>✓</Text>}
            </View>
            <Text style={[styles.checkLabel, isPurchased && styles.checkLabelSel]}>
              {ro ? 'Cumparat (nu din camp)' : 'Purchased (not from field)'}
            </Text>
          </TouchableOpacity>
          ...
          {loading
            ...
            : (
              <TouchableOpacity style={styles.addBtn} onPress={handleSubmit} activeOpacity={0.8}>
                <Text style={styles.addBtnTxt}>{ro ? 'Adauga la Ferma' : 'Add Bales to Farm'}</Text>
              </TouchableOpacity>
            )
          }
        </ScrollView>
