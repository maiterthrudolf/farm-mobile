...
import { searchPrepareAnimalByLast4, InseminationAnimalInfo } from '../api/client';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'InseminationPrepareScan'>;
};

export default function InseminationPrepareScanScreen({ navigation }: Props) {
  const { lang } = useLang();
  const ro = lang === 'RO';
  ...
  const proceed = (animal: InseminationAnimalInfo) => {
    navigation.navigate('InseminationPrepareDetails', { animal });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const animals = await searchPrepareAnimalByLast4(last4);
      if (animals.length === 0) {
        setError(ro ? 'Crotaliu nu este valid [mort/vandut/gestant]' : 'No valid ID [dead/sold/pregnant]');
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <NavHeader title={ro ? 'Pregatire insamintare artificiala' : 'Preparation of artificial insemination'} onBack={() => navigation.navigate('InseminationMenu')} />
