...
import { searchPregnancyAnimalByLast4, PregnancyAnimalInfo } from '../api/client';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'PregnancyScan'>;
};

export default function PregnancyScanScreen({ navigation }: Props) {
  ...
  const [results, setResults] = useState<PregnancyAnimalInfo[] | null>(null);
  ...
  const proceed = (animal: PregnancyAnimalInfo) => {
    navigation.navigate('PregnancyDetails', { animal });
  };
  ...
  const handleSearch = async (raw: string) => {
    ...
    setLoading(true);
    try {
      const animals = await searchPregnancyAnimalByLast4(last4);
      if (animals.length === 0) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <NavHeader title={t('pregnancy')} onBack={() => navigation.goBack()} />
